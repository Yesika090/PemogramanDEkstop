...
Public Class Form1
    ...
    Private Sub DeleteFile(ByVal filePath As String)
        Try
            ' Membuat objek untuk file yang akan dihapus
    ...
    Private Sub DeleteFileToolStripMenuItem_Click(sender As Object, e As EventArgs) Handles DeleteFileToolStripMenuItem.Click
        If lvFile.SelectedItems.Count = 1 Then
            ...
            DeleteFile(filePath)
            For Each item As ListViewItem In lvFile.Items
                If item.Tag IsNot Nothing AndAlso item.Tag.ToString() = filePath Then
                    lvFile.Items.Remove(item)
                    Exit For ' Hentikan iterasi setelah item dihapus
                End If
            Next
        End If
    End Sub
    ...
    Private Sub RenameFile(ByVal filePath As String)
        Try
            ...
            If fileInfo.Exists Then
                ...
                Dim newFileName As String = InputBox("Enter new file name:", "Rename File", fileInfo.Name)

                If newFileName <> "" Then
                    Dim fileDirectory As String = fileInfo.Directory.FullName
                    ...
                    If Not System.IO.File.Exists(newFilePath) Then
                        ...
                        ' Memperbarui tampilan di ListView
                        For Each item As ListViewItem In lvFile.Items
                            If item.Tag IsNot Nothing AndAlso item.Tag.ToString() = filePath Then
                                item.Tag = newFilePath
                                item.SubItems(0).Text = newFileName
                                Exit For
                            End If
                        Next
                    End If
